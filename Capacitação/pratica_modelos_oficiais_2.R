...
#   (+ o programa JAGS instalado no sistema operacional -- nao é pacote R,
#    necessario pro script original do CMSY++: https://sourceforge.net/projects/mcmc-jags/)

#diretorio
setwd("C:/Users/mathe/OneDrive/Documents/Cabo-Verde---Stock-Assessment/Capacitação")
# =========================================================================
# DADOS DE EXEMPLO
# =========================================================================
# Serie de 31 anos (1950-1980) -- a mesma dos slides de DB-SRA e CMSY++
anos    <- 1950:1980
captura <- c(100,150,200,300,400,500,600,900,1000,1500,2000,2500,3000,
             800,500,400,500,300,200,900,2000,2500,4500,4000,2000,1800,
             900,600,300,200,80)

# Exemplo de 15 anos com captura E cpue -- o mesmo do JABBA nas slides
# (o JABBA PRECISA de um índice de abundância; DB-SRA e CMSY++, não)
anos15    <- 1:15
captura15 <- c(200,400,700,1000,1300,1400,1300,1200,1100,1000,900,850,800,780,760)
cpue15    <- c(9.62,9.10,9.57,9.19,7.64,7.15,6.91,6.35,6.15,5.80,6.17,6.21,6.15,6.57,6.56)


# =========================================================================
# PARTE 1 -- DB-SRA oficial: fishmethods::dbsra()
# =========================================================================
# Cada argumento é uma distribuição de onde o pacote sorteia aquele
# parâmetro a cada rodada Monte Carlo (nsims rodadas). É a MESMA ideia dos
# slides (sorteia M, Fmsy/M, Bmsy/K, Bt/K e procura K), só que o pacote faz
# a busca de K e a checagem de viabilidade por dentro.
#
# btk$refyr = o ano de referência para a depleção-alvo (aqui, 1980, o
# último ano da série -- equivalente ao "Bt/K = 0,30 em 1980" das slides).

library(fishmethods)

res_dbsra <- dbsra(
  year = anos, catch = captura,
  agemat = 2,                                    # idade de maturação (anos)
  k     = list(low = 5000, up = 100000, tol = 0.01, permax = 1000),
  b1k   = list(dist = "unif",  low = 0.8,  up = 1.0,  mean = 1,      sd = 0.1),
  btk   = list(dist = "beta",  low = 0.2,  up = 0.4,  mean = 0.30,  sd = 0.1,  refyr = 1980),
  fmsym = list(dist = "lnorm", low = 0.3,  up = 1.6,  mean = log(0.8), sd = 0.2),
  bmsyk = list(dist = "beta",  low = 0.05, up = 0.95, mean = 0.40,  sd = 0.1),
  M     = list(dist = "lnorm", low = 0.05, up = 0.6,  mean = log(0.2), sd = 0.4),
  nsims = 5000,
  grout = 1                                       # 1 = plota o resumo ao final
)

cat("\n== DB-SRA (fishmethods::dbsra) ==\n")
print(res_dbsra$Values)   # tabela-resumo (mediana e IC das quantidades-chave)
# res_dbsra$detailedOutput traz TODAS as 5.000 rodadas (aceitas e rejeitadas),
# caso vocês queiram reconstruir a nuvem de K aceito, como no slide do DB-SRA.
...
## --- 2.1 a série de captura, no formato que o script espera (ct$year, ct$ct)
ct <- data.frame(year = anos, ct = captura)

## --- 2.2 hipótese de r (mesmo formato do r_macarellus.csv de vocês) -----
## Uma única hipótese aqui, de propósito -- "simplória": resiliência
## Média (Froese et al. 2017), a mesma faixa usada em todas as slides.
## Pra testar mais de uma hipótese (como vocês fazem: Euler-Lotka, lower/
## higher resilience, non-informative...), é só empilhar mais linhas aqui.
r_exemplo <- data.frame(
  especie  = "Exemplo_curso",
  ano      = "1950-1980",
  hipotese = "Media_resiliencia",
  metodo   = "Tabela de resiliencia (Froese et al. 2017)",
  fonte    = "Exemplo didatico",
  racional = "Resiliencia media -> r entre 0,2 e 0,8, a mesma faixa usada nas slides.",
  r_lo = 0.2, r_hi = 0.8, r = 0.47
)

## --- 2.3 hipótese de B/k final (mesmo formato do bk_macarellus.csv) -----
## Também uma única linha: a mesma janela de depleção final usada no
## exemplo de DB-SRA (Bt/K = 0,30 em 1980), pra os três métodos contarem
## a mesma história.
bk_exemplo <- data.frame(
  especie  = "Exemplo_curso",
  ano      = 1980,
  hipotese = "Alvo_do_exemplo",
  metodo   = "Mesmo alvo usado no DB-SRA das slides",
  fonte    = "Exemplo didatico",
  racional = "Depleção final entre 0,20 e 0,40 (centro 0,30), igual ao Bt/K do exemplo de DB-SRA.",
  bk_lo = 0.2, bk_hi = 0.4, bk = 0.3
)

## =========================================================================
## A PARTIR DAQUI: o código de vocês, sem alterações
## =========================================================================
# Combinação completa: cada bk com cada r
cenarios_exemplo_cmsy <- merge(
  bk_exemplo,
  r_exemplo,
  by = "especie",
  suffixes = c("_bk", "_r")
...
stocks_exemplo <- cenarios_exemplo_cmsy %>%
  mutate(
...
cinfo <- cenarios_exemplo_cmsy %>%
  mutate(
    ...
    stock_base = especie,
    category = especie,
    region = "Exemplo didatico",
    source = "Curso de Avaliacao de Estoques",
    #--------------------------------------------------------
    # Cobertura tempora
    #--------------------------------------------------------
    init_yr = min(ct$year),
    ...
    #--------------------------------------------------------
    Continent = NA, Region = NA, Subregion = NA,
    #--------------------------------------------------------
    # INformação taxonomica
    #--------------------------------------------------------
    Group = NA, Name = "Exemplo do curso", ScientificName = "Exemplo_curso",
    SpecCode = NA, Source = "Curso de Avaliacao de Estoques",
    #--------------------------------------------------------
    # Periodo de capturas
    #--------------------------------------------------------
    MinOfYear = init_yr,
    ...
    # Depleção inicial
    #--------------------------------------------------------
    stb.low = 0.8,   # 1950 quase virgem (igual ao exemplo de DB-SRA: B1950 = K)
    stb.hi = 1,
    #--------------------------------------------------------
    ...
    #--------------------------------------------------------
    btype = "None",   # "None" = só captura, sem índice -- como no exemplo de CMSY++ das slides
    e.creep = NA,
    force.cmsy = TRUE,
    Comments = NA) %>%
...
file1  <- "cdat_exemplo_cmsy.csv"
write.table(cdat, file = file1, append =FALSE,dec=".",sep = ",",
            row.names = FALSE)
# Salvar como Excel
write_xlsx(cdat,  path = "cdat_exemplo_cmsy.xlsx")

file2  <- "cinfo_exemplo_cmsy.csv"
...
#---------------------------------------------
catch_file  <- "cdat_exemplo_cmsy.csv" #"Stocks_Catch_2020CMSYrun2_v5_RS - Copy.csv"  #"CombStocks_Catch_2020CMSYrun3_v4.csv"  # "SAUP_Catch_1.csv"  #"SimCatchCPUE_4.csv"  # "Stocks_Catch_Aust_2.csv" #"STECF_Catch_2020_2.csv" #"tRFMO_Catch_2020.csv" #"ICES_Catch_2020.csv" #"Global_Stocks_Catch.csv" #"SimCatchCPUE_4.csv" #"Stocks_Catch_Test.csv"  #"Stock_Catch_forRainer.csv" # "SimCatchCPUE_3.csv" #  name of file containing "Stock", "yr", "ct", and optional "bt"
id_file     <- "cinfo_exemplo_cmsy.csv" #"Stocks_ID_forDeng_RSb - Copy.csv" # "CombStocks_ID_2020CMSYrun3_v3_RF3.csv"   #    "SAUP_ID_2.csv"    #"SimSpecCPUE_4_NA_int_end.csv"  #"Train_ID_7j.csv" # "Stocks_ID_Aust_4.csv"  #"STECF_ID_2020_2.csv" #tRFMO_ID_2020_2.csv" #"ICES_ID_2020_4.csv" #"Robust_Stocks_ID_10_allNA.csv" #"SimSpecCPUE_4.csv"#"Stocks_ID_R_7.csv"  #"Stock_ID_forRainer.csv"  #  "NCod_ID_4.csv" #"SimSpecCPUE_3.csv" #  name of file containing stock-specific info and settings for the analysis
...
write.table(bio_out, file = "bio_out_exemplo_cmsy.csv", 
            dec=".",sep = ",", row.names = FALSE) 
# Salvar como Excel
write_xlsx(bio_out,  path = "bio_out_exemplo_cmsy.xlsx")


#write r-k samples priors/posteriors
write.table(rk_out, file = "rk_out_exemplo_cmsy.csv", 
            dec=".",sep = ",", row.names = FALSE) 
# Salvar como Excel
write_xlsx(rk_out,  path = "rk_out_exemplo_cmsy.xlsx")


#write the prior posterior data intervals
write.table(cmsy_out, file = "cmsy_out_exemplo_cmsy.csv", 
            dec=".",sep = ",", row.names = FALSE) 
# Salvar como Excel
write_xlsx(cmsy_out,  path = "cmsy_out_exemplo_cmsy.xlsx")


#write kobe out with f/fmsy and b/bmsy series
write.table(kobe_out, file = "kobe_out_exemplo_cmsy.csv", 
            dec=".",sep = ",", row.names = FALSE) 
# Salvar como Excel
write_xlsx(kobe_out,  path = "kobe_out_exemplo_cmsy.xlsx")
...
# =========================================================================
# PARTE 3 -- JABBA oficial: build_jabba() + fit_jabba()
# =========================================================================
# JABBA PRECISA de um índice de abundância (cpue); por isso usa o exemplo
# de 15 anos, não a série de 31 anos das outras duas partes.

library(JABBA)

capt <- data.frame(Yr = anos15, Catch = captura15)
cpue <- data.frame(Yr = anos15, cpue1 = cpue15)   # uma coluna por índice; aqui só 1

jbin <- build_jabba(
  catch        = capt,
  cpue         = cpue,
  assessment   = "exemplo_curso",
  scenario     = "toy15",
  model.type   = "Schaefer",
  add.catch.CV = TRUE,
  catch.cv     = 0.1,
  r.dist       = "range",             # converte um intervalo (min,max) numa lognormal
  r.prior      = c(0.2, 0.8),         # mesma faixa de resiliência "Medium" do CMSY++
  K.dist       = "range",
  K.prior      = c(5000, 40000),
  psi.dist     = "lnorm",             # psi = biomassa relativa no 1o ano (B1/K)
  psi.prior    = c(0.95, 0.25),       # media, CV -- quase virgem em 1966 (ano 1 do exemplo)
  sigma.est    = TRUE,                # estima o erro de observacao da CPUE
  sets.q       = 1,                   # 1 indice = 1 q
  sets.var     = 1,
  catch.metric = "(t)",
  verbose      = TRUE
)
...
fit <- fit_jabba(jbin,
                  ni = 15000, nb = 3000, nt = 3, nc = 2,   # iteracoes, burn-in, thin, cadeias
                  quickmcmc  = TRUE,   # TRUE = mais rapido p/ pratica; FALSE pra avaliacao de verdade
                  save.jabba = FALSE, save.csvs = FALSE,
                  verbose = FALSE)

cat("\n== JABBA (build_jabba + fit_jabba) ==\n")
cat("MSY      :", round(fit$refpts$msy[1], 1), "\n")
cat("Bmsy     :", round(fit$refpts$bmsy[1], 1), "\n")
cat("B/Bmsy final:", round(fit$timeseries[nrow(fit$timeseries), "mu", "BBmsy"], 3), "\n")
cat("F/Fmsy final:", round(fit$timeseries[nrow(fit$timeseries), "mu", "FFmsy"], 3), "\n")

## --- 3.1 os graficos padrao, todos de uma vez -----------------------
## jabba_plots() e a funcao "faz tudo": escreve, de uma vez, PNGs com o
## ajuste aos indices (observado x previsto), residuos, teste de runs,
## distribuicoes prior x posterior, trajetorias de biomassa/F, fase de
## producao e o grafico de Kobe -- e o mesmo painel completo que sai de
## cada cenario nos ajustes reais de voces.
dir_figuras <- "figuras_jabba_exemplo"
dir.create(dir_figuras, showWarnings = FALSE)
jabba_plots(jabba = fit, output.dir = dir_figuras)
cat("Figuras do JABBA salvas em:", dir_figuras, "\n")

## --- 3.2 se vc quiser só UM grafico especifico, em vez do pacote todo,
##     cada peca do jabba_plots() acima tambem existe solta:
# jbplot_catch(fit)        # captura observada x captura ajustada
# jbplot_cpuefits(fit)     # cpue observada x cpue prevista (o q*B do slide 52)
# jbplot_ppdist(fit)       # prior x posterior de r, K, psi, sigma...
# jbplot_trj(fit, type = "BB0")   # trajetoria de B/B0 (ou "FFmsy", "BBmsy", "Bt")
# jbplot_procdev(fit)      # desvios do erro de processo, ano a ano
# jbplot_spphase(fit)      # fase de producao (equivalente ao painel F do CMSY++)
# jbplot_kobe(fit)         # grafico de Kobe
# jbplot_runstest(fit)     # teste de runs nos residuos (diagnostico de ajuste)
#
## --- 3.3 comparando VARIOS cenarios/ajustes num so painel -----------
## so faz sentido com mais de um fit; passa uma LISTA nomeada de ajustes:
# fits <- list(cenario_A = fit, cenario_B = fit_outro_cenario)
# jbplot_summary(fits, as.png = TRUE, output.dir = dir_figuras)
